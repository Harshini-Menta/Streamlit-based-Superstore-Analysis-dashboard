...
st.title(" :shopping_bags: Sample SuperStore EDA")
...
if file is not None:
    ...
    os.chdir("/home/harshini/Desktop/streamlit/")
...
with col1:
    ...
    fig = px.bar(category_df, x = "Category", y = "Sales",color="Category", text = ['${:,.2f}'.format(x) for x in category_df["Sales"]],
                 template = "seaborn",color_discrete_sequence=px.colors.qualitative.Pastel)
    st.plotly_chart(fig,use_container_width=True, height = 200)

with col2:
    st.subheader(":earth_africa: Sales Distribution by Region")
    fig = px.pie(filtered_df, values = "Sales", names = "Region", hole = 0.5,color_discrete_sequence=px.colors.sequential.Blues)
